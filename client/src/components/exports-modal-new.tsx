      ...
      try {
        ...
        employees.forEach((employee, index) => {
          ...
          if (nightHours.details.length > 0) {
            nightHours.details.forEach((detail, detailIndex) => {
              const detailText = `${formatDate(new Date(detail.date))}: ${detail.hours.toFixed(2)}h`;
              pdf.text(detailText, pageWidth - margin - 50, currentY + (detailIndex * 5));
              
              // Solo incrementamos Y después del último detalle
              if (detailIndex === nightHours.details.length - 1) {
                currentY += Math.max(5, nightHours.details.length * 5) + 2;
              }
            });
          } else {
                            ...
                            <div>
                              {nightHours.details.map((detail, idx) => (
                                <div key={idx} className="mb-1">
                                  {formatDate(new Date(detail.date))}: {detail.hours.toFixed(2)}h
                                </div>
                              ))}
                            </div>
                          ) : (
